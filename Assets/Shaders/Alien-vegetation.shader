Shader "IOX/Alien-vegetation"
{
	Properties
	{
		...
		_TranslucencyColor("Translucency Color", Color) = (1,0,0,1)
		_RimAmount("Rim Lighting Amount", Float) = .2
	...
	SubShader
	{
		...
		Pass
		{
			...
			FragmentOutput frag(v2f i)
			{
				...
				half3 emission = tex2D(_EmissionTex, i.uv) * _EmissionColor;
				emission = exp2(-emission.rgb * _EmissionStrength);

				FragmentOutput output;
				output.gBuffer0 = float4(ctex * _Color * i.color, _Shine / 64);
				output.gBuffer1 = float4(ctex * _TranslucencyColor * i.color, _Power / 256);
				output.gBuffer2 = float4(normal * .5 + .5 , _RimAmount);
