...
public class CollidableShape
{
    ...
    private void InitializeShapes()
    {
        ...
        foreach (ConvexShape shape in ConvexShapes)
        {
            ...
            foreach (Vector2 shapePoint in shape.ShapePoints)
            {
                ...
                {
                    EdgeLine = new Line2(previousPoint, position + shapePoint * scale)
                };
        ...
        if (CornerPoints.Count > 1)
        {
            ...
            for (int i = 0; i < CornerPoints.Count; i++)
            {
                ...
                {
                    LeftPosition = previousCorner,
                    Position = position + CornerPoints[i] * scale,
                    RightPosition = position + CornerPoints[i == CornerPoints.Count - 1 ? 0 : i + 1] * scale
                };
    ...
    public Vector2 GetBestNextCorner(Vector2 from, Vector2 target)
    {
        ...
        if (GetATanCorners(target, out Vector2 leftCornerTo, out Vector2 rightCornerTo))
    ...
    public Vector2 GetBestInitialCorner(Vector2 from, Vector2 target)
    {
        ...
        if (GetATanCorners(from, out Vector2 leftCornerFrom, out Vector2 rightCornerFrom))
        {
            //get difference to corners
            Vector2 fromLeftToTarget = target - leftCornerFrom;
            Vector2 fromRightToTarget = target - rightCornerFrom;

            if (GetATanCorners(target, out Vector2 leftCornerTarget, out Vector2 rightCornerTarget))
            {
