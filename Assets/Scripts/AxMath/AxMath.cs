...
public static class AxMath
{
    ...
    public static int OneThousanth(int amount)
    {
        double v = amount;
    ...
    public static int Atan2Direction(Vec2I origo, Vec2I target)
    {
        ...
        return Mathf.RoundToInt((SafeAtan2(target.y - origo.y, target.x - origo.x) + Mathf.PI) / TAU * 8) % 8 + 1;
    }
    ...
    //bresenham line with floating point values
    ...
    {
        List<Vector2> original = new List<Vector2>(vertices);

        for (int i = vertices.Count - 1; i >= 0; i--)
            for (int j = original.Count - 1; j >= 0; j--)
                if (vertices[i] == original[j])
                    if (i != j)
                    {
                        vertices.RemoveAt(i);
                        break;
